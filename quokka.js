(function (Scratch) {
  ...
  class QasmBuilder {
    constructor(runtime) {
      ...
      console.log('✅ QasmBuilder constructor, runtime ok?', !!runtime);
    }
      ...
      return {
        ...
        blocks: [
          ...
          },
          {
            ...
            text: 'qreg [NAME] [SIZE]',
            arguments: {
              NAME: { type: Scratch.ArgumentType.STRING, defaultValue: 'q' },
          ...
          },
          {
            ...
            text: 'creg [NAME] [SIZE]',
            arguments: {
              NAME: { type: Scratch.ArgumentType.STRING, defaultValue: 'c' },
              SIZE: { type: Scratch.ArgumentType.NUMBER, defaultValue: 1 }
            }
          },
          {
          ...
          },
          {
            ...
            text: 'RX ( [ANGLE] ) q [Q]',
            arguments: {
              ANGLE: { type: Scratch.ArgumentType.NUMBER, defaultValue: Math.PI },
              Q:     { type: Scratch.ArgumentType.NUMBER, defaultValue: 0 }
            }
          },
          {
          ...
          },
          {
            opcode: 'runQuantum',
            blockType: Scratch.BlockType.COMMAND,
            text: 'run QASM with [SHOTS] shots',
            arguments: {
    ...
    hGate({ Q }) {
      this.lines.push(`h q[${Q}];`);
    }
    xGate({ Q }) {
      this.lines.push(`x q[${Q}];`);
    }
    cxGate({ Q1, Q2 }) {
      this.lines.push(`cx q[${Q1}],q[${Q2}];`);
    }
    ccxGate({ C1, C2, T }) {
      this.lines.push(`ccx q[${C1}],q[${C2}],q[${T}];`);
    }
    rxGate({ ANGLE, Q }) {
      this.lines.push(`rx(${ANGLE}) q[${Q}];`);
    }
    ryGate({ ANGLE, Q }) {
      this.lines.push(`ry(${ANGLE}) q[${Q}];`);
    }
    rzGate({ ANGLE, Q }) {
      this.lines.push(`rz(${ANGLE}) q[${Q}];`);
    }

    measure({ Q, C }) {
      this.lines.push(`measure q[${Q}] -> c[${C}];`);
    }
    ...
    runQuantum({ SHOTS }) {
      ...
      fetch('https://quokka2.quokkacomputing.com/qsim/qasm', {
        ...
        .then(json => {
          ...
          this.runtime.startHats('qasmBuilder_whenResults');
        })
        .catch(err => {
  ...
  // Register the class so Scratch passes you a real `runtime` object
  Scratch.extensions.register(QasmBuilder);
})(window.Scratch);
